...
git tag -l | while read TAG ; do
    ...
    for PLATFORM in Python MicroPython RaspberryPi esp8266 esp32 esp8266_sim800; do
    ...
    # Do we have to zip the installer as well? --> we don0t do it here anymore...
    #echo "Checking if zip the installer for $DEST/PythingsOS/$TAG/tools/installer"
    #if [ -d "$DEST/PythingsOS/$TAG/tools/installer" ]; then
    #    if grep -q $TAG "$DEST/PythingsOS/$TAG/tools/installer/selfarchive.py"; then
